...
export default function ProjectReturnMaterialRequestsPage() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<ProjReturnMaterialRequest | null>(null);
  ...
  const handleIssueFromRequest = (request: ProjReturnMaterialRequest) => {
  ...
  const handleApproveRequest = async (requestId: string) => {
    try {
      const response = await fetch(`/api/projectreturn-materials/requests/${requestId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const columns: ColumnDef<ProjReturnMaterialRequest>[] = [
    {
  ...
  return (
    <div className="container mx-auto p-4 min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Project Return Material Requests</h1>
      </div>
